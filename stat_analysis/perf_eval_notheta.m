...
% only for model variants without theta module iModel = (2, 4, 6) <=> B, D, F
% model variants
N_model = 7; 
...
for iModel = 2 :  2 : N_model
    fpath = fullfile(dataFold, ['reduced_variant_' num2str(iModel)]);
	...
	for iSentence = 1 : N_sentence
        ...
        syllable_units = vv2(:, 9 : end); % with the silent unit
        N_syl = size(vv2,2) - 8; % with silent unit
...
N_tot_syl = 2*N_tot_syl/N_model;

% only columns corresponding to the models without theta module would be populated.
fname = 'perf_internal_no_theta';
